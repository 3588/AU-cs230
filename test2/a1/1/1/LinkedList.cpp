...
////////////////////////////////////function test2
template <class NodeType>
void LinkedList<NodeType>::divideAt( LinkedList<NodeType> &secondList, const NodeType & item, int &del) 
// Divide the list at the node with the data item into two sublists. 
// Postcondition: firstNode and lastNode point to the first and last 
//    nodes of the first sublist. 
//    secondList.firstNode and secondList.lastNode point to 
//      the first and last nodes of the second sublist. 
{
	if( isListEmpty())
	{
		cout << "List is empty." << endl;
	...
	ListNode<NodeType>* currentNode = firstNode;
	ListNode<NodeType>* mylist=lastNode;
	int sendto2nd=0;
	while( currentNode != NULL)
	{
		if ((currentNode -> data==item)&&(sendto2nd==0))
		{
			sendto2nd++;//fund the item, put data to secondlist and del data
		//secondList.insertAtBack(item);
		}
		if (sendto2nd)
		{secondList.insertAtBack(currentNode -> data);
		sendto2nd++;
		}
		currentNode = currentNode -> nextNodePtr;
	}
